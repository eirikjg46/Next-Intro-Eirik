...
const fakePrice: number[] = [499, 999, 399, 229, 159]

// Define an array of fake categories.
const fakeCategories: string[] = ["Book", "Art", "Hardware"]
...
export const faker: Faker = {
  ...
  // Get a random title option from the fakeAnswers array.
  title: () => getRandomItem(fakeTitle),

  // Get a random price from the fakeScores array.
  price: () => getRandomItem(fakePrice),
...
const createResponses: CreateResponses = ({
  ...
  for (let i = 0; i < count; i++) {
    ...
    const response = {
      price: faker.price(), // Get a random score.
      category: faker.category(), // Get a random category.
      title: faker.title(), // Get a random answer option.
    }
